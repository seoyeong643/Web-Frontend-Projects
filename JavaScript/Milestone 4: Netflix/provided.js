...
const _showTemplate = `<div class="col {{colSize}}">
      <div class="card">
        <img src="{{imageSrc}}" class="card-img-top" alt="{{title}}">
      </div>
    </div>`;

const _showRowElement = document.getElementById("show-row");

const _selectedGenre = document.getElementById("selected");
...
function displayShow(show) {
  ...
  let newColumn = _showTemplate
    .replaceAll("{{imageSrc}}", showObj.img)
    .replaceAll("{{title}}", showObj.title)
    .replaceAll("{{colSize}}", "col-7");
...
function displayAllShows() {
  ...
  for (let i = 0; i < SHOWS.length; i++) {
    let newCard = _showTemplate
      .replaceAll("{{imageSrc}}", SHOWS[i].img)
      .replaceAll("{{title}}", SHOWS[i].title)
      .replaceAll("{{colSize}}", "");
    _showRowElement.insertAdjacentHTML("beforeend", newCard);
  }
}
